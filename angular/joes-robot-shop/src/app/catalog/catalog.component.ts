...
export class CatalogComponent {
  ...
  /*
  In Angular, the getFilteredProducts() method is called automatically whenever Angular detects a change in the component's state or input bindings.

When you click on one of the buttons (e.g., "Heads"), the (click) event binding in the HTML triggers a change in the filter variable of the component. This change in the variable's value is detected by Angular's change detection mechanism.

Angular then automatically re-evaluates any expressions in the template that depend on the changed variable. In this case, the *ngFor directive in your HTML template depends on the result of the getFilteredProducts() method.

Since the value of filter has changed, Angular re-runs the getFilteredProducts() method to obtain the updated list of filtered products. The *ngFor directive then iterates over this updated list of products, generating the necessary HTML elements to display each product in the UI.

So, clicking the button triggers a change in the filter variable, which in turn triggers the re-evaluation of the getFilteredProducts() method and the subsequent re-rendering of the product list in the UI through the *ngFor directive.
  */
  getFilteredProducts() {
    return this.filter === ''
      ? this.products
